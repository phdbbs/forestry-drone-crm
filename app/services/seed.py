...
def seed_if_empty():
    ...
    db.session.flush()
    ct1 = Contact(name="王处长", customer_id=c1.id, title="林业管理处处长", phone="138-0571-1234", email="wang@zjly.gov.cn", wechat="wangzjly", importance="关键", business_scope="松材线虫病防控、林业无人机巡检")
    ct2 = Contact(name="李科长", customer_id=c2.id, title="科技处处长", phone="139-0591-5678", email="li@fjly.gov.cn", wechat="li_fjly", importance="关键", business_scope="森林防火监测、智慧林业")
    ct3 = Contact(name="张主任", customer_id=c3.id, title="资源处处长", phone="137-0791-9012", email="zhang@jxly.gov.cn", wechat="zhangjx", importance="重要", business_scope="森林资源普查、航拍核查")
    ct4 = Contact(name="刘经理", customer_id=c4.id, title="政企客户部经理", phone="136-0571-3456", email="liu@china-tower.com", wechat="liutower", importance="重要", business_scope="无人机通信中继")
    ct5 = Contact(name="周教授", customer_id=c6.id, title="教授/AI实验室主任", phone="139-8711-2233", email="zhou@swfu.edu.cn", wechat="zhou_ai", importance="重要", business_scope="AI病虫害算法")
    db.session.add_all([ct1, ct2, ct3, ct4, ct5])
    db.session.flush()
    l1 = Lead(title="2026年浙江省松材线虫病防治无人机巡检项目", bid_number="ZJ-2026-0342", budget="150", deadline=datetime(2026,8,15), region="浙江省", purchaser="浙江省林业局", service_content="松材线虫病无人机飞行检查、林区巡检、航拍核查、AI病虫害算法识别", match_keywords="松材线虫病防治,无人机飞行检查,林区巡检,航拍核查,AI病虫害算法识别", match_level="高匹配", match_reason="包含高匹配关键词", source_platform="中国政府采购网", source_url="#", status="active", customer_id=c1.id)
    l2 = Lead(title="福建省森林防火无人机监测服务采购", bid_number="FJ-2026-0189", budget="280", deadline=datetime(2026,7,30), region="福建省", purchaser="福建省林业厅", service_content="森林防火无人机监测、热成像监测、应急通信保障", match_keywords="无人机监测,森林,监测", match_level="中匹配", match_reason="匹配关键词", source_platform="全国公共资源交易平台", source_url="#", status="active", customer_id=c2.id)
    l3 = Lead(title="江西省林区航拍核查服务", bid_number="JX-2026-0056", budget="90", deadline=datetime(2026,6,22), region="江西省", purchaser="江西省林业局", service_content="林区航拍核查、森林资源普查", match_keywords="林区航拍核查,航拍,林区", match_level="高匹配", match_reason="包含高匹配关键词", source_platform="江西省公共资源交易中心", source_url="#", status="active", customer_id=c3.id)
    l4 = Lead(title="四川省松材线虫病林木砍伐清运施工", bid_number="SC-2026-0421", budget="65", deadline=datetime(2026,9,1), region="四川省", purchaser="四川省林业和草原局", service_content="松材线虫病林木砍伐、清运、消杀施工", match_keywords="林木砍伐,清运,消杀施工", match_level="低匹配", match_reason="仅含低匹配关键词", source_platform="中国政府采购网", source_url="#", status="active", customer_id=None)
